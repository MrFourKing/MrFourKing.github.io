...
var initHeader = function initHeader() {
  ...
  var breakpointLg = window.matchMedia('(min-width: 767px)');
  if (!header) return;
  var burgerBtn = header.querySelector('.header__toggle');
  var navBlock = header.querySelector('.header__nav');
  var navLinks = navBlock.querySelectorAll('.main-menu__link');
  var logoLink = header.querySelector('.header__logo');
  ...
  var initLinksAction = function initLinksAction() {
    navLinks.forEach(function (item) {
      item.addEventListener('click', function (evt) {
        var target = item.dataset.target === '#start' ? 0 - pageYOffset : document.querySelector(item.dataset.target);
        evt.preventDefault();
        closeMenu();
        moveTo.move(target);
      });
